...
const createTrip = ({loaderData}: Route.ComponentProps) => {
  ...
  return (
    <main className='flex flex-col gap-10p b-20 wrapper'>
        <Header
        ...
        <section className='mt-2.5 wrapper-md'>
            <form className='trip-form ' onSubmit={handleSubmit}>
                ...
                <ComboBoxComponent 
                    id="country"
                    dataSource={countries as Record<string, any>[]}
                    fields={{ text: "name", value: "value" }}
                    placeholder='Select a Country'
                    className='combo-box font-emoji'
                    itemTemplate={(data: any) => (
                        <div className="flex items-center pl-3">
                        <img src={data.flagUrl} alt={data.name} className="w-5 h-3 object-cover rounded-sm" />
                        <span>{data.name}</span>
                        </div>
                    )}
